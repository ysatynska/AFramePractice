...
AFRAME.registerComponent('camera-boost', {
  schema: {
    acceleration: { default: 50 }
  },

  init: function () {
    // Use the value from the schema
    const accel = this.data.acceleration;
    // Set the wasd-controls attribute
    this.el.setAttribute('wasd-controls', `acceleration: ${accel}`);
...
AFRAME.registerComponent('simple-navmesh-constraint', {
  ...
  onSceneUpdated: function (evt) {
    ...
    if (!navmeshSelector && !excludeSelector) { return; }
  
    // If we have a navmesh selector, check it
    if (navmeshSelector && navmeshSelector.length > 0 && evt.detail.el.matches(navmeshSelector)) {
      this.entitiesChanged = true;
    }
  
    // If we have an exclude selector, check it
    if (excludeSelector && excludeSelector.length > 0 && evt.detail.el.matches(excludeSelector)) {
      this.entitiesChanged = true;
    }
  },
  ...
  updateNavmeshEntities: function () {
    ...
    // 1) Instead of using this.data.navmesh, we use #segmented-plane
    const planeEl = document.querySelector('#segmented-plane');
    if (!planeEl) {
      console.warn('simple-navmesh-constraint: #segmented-plane not found in DOM!');
    } else {
      // 2) Push its THREE.Object3D into this.objects
      this.objects.push(planeEl.object3D);
    }
  
    // dont need this for now, but eh
